...
$("logout-main").onclick = () => signOut(auth);
$("logout-results").onclick = () => signOut(auth);
...
onAuthStateChanged(auth, async (user) => {
  ...
  const logoutMainBtn = document.getElementById('logout-main');

  if(user){
    userId = user.uid;
    userName = user.displayName || user.email || "User";
    document.getElementById("hello").textContent = `Hi ${userName}!`;

    // Reset UI to show main menu, hide results view
    mainMenuContent.style.display = 'block';
    resultsPanel.style.display = 'none';
    resultsButtons.style.display = 'none';
    resultBtn.style.display = 'inline-block';
    logoutMainBtn.style.display = 'inline-block';
...
async function beginQuizFromUrl(url, subjectLabel, chapterTitle){
  ...
  $("welcome-banner").innerHTML =
    `Welcome <span class="name">${userName}</span> in Cyber Quiz App of <span class="name"><b>${subjectLabel}</b></span>`;
  show("quiz");
